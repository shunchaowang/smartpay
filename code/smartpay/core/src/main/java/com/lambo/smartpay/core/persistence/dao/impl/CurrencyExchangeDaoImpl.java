...
public class CurrencyExchangeDaoImpl extends GenericDaoImpl<CurrencyExchange, Long>
    ...
    private static final Logger logger = LoggerFactory.getLogger(CurrencyExchangeDaoImpl.class);

    /**
     * Dynamic search like grails findBy...
     * We create a dynamic criteria, like grails createCriteria() {}.
     * There are two parts of the search to support grails criteria search with DataTables
     * instant
     * search.
     *
     * @param currencyExchange contains all criteria for equals, like name equals xx and
     *                         active equals
     *                         true, etc.
    ...
    public Long countByCriteria(CurrencyExchange currencyExchange, String search) {
        ...
        Root<CurrencyExchange> root = query.from(CurrencyExchange.class);
        query.select(builder.count(root));

        Predicate predicate = null;
        // if customer is not null equal predicate needs to be generated
        if (!isBlank(currencyExchange)) {
            predicate = equalPredicate(builder, root, currencyExchange);
        }
     ...
     */
    @Override
    public List<CurrencyExchange> findByCriteria(CurrencyExchange currencyExchange,
                                                 String search,
                                                 Integer start, Integer length, String order,
                                                 ResourceProperties.JpaOrderDir orderDir) {
        CriteriaBuilder builder = entityManager.getCriteriaBuilder();
        CriteriaQuery<CurrencyExchange> query = builder.createQuery(CurrencyExchange.class);
        Root<CurrencyExchange> root = query.from(CurrencyExchange.class);
        query.select(root);
        ...
        TypedQuery<CurrencyExchange> typedQuery = entityManager.createQuery(query);
        // pagination
        if (start != null) {
     ...
     *
     * @param currencyExchange null return false, all required fields are null return false.
     * @return
     */
    @Override
    public Boolean isBlank(CurrencyExchange currencyExchange) {
        return currencyExchange == null || currencyExchange.getCurrencyFrom() == null;
    }
     ...
     */
    private Predicate likePredicate(CriteriaBuilder builder, Root<CurrencyExchange> root, String
            search) {

        String likeSearch = "%" + search + "%";

        // get all paths for the query
        Path<String> currencyFromPath = root.join("currencyFrom").<String>get("name");

        Predicate predicate = builder.like(currencyFromPath, likeSearch);
     ...
     * @param builder          is the JPA CriteriaBuilder.
     * @param root             is the root of the CriteriaQuery.
     * @param currencyExchange is the search keyword.
     * @return JPA Predicate used by CriteriaQuery.
     */
    private Predicate equalPredicate(CriteriaBuilder builder, Root<CurrencyExchange> root,
                                     CurrencyExchange
                                             currencyExchange) {
     ...
     */
    private Order orderBy(CriteriaBuilder builder, Root<CurrencyExchange> root,
